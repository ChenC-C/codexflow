...
const MethodChannel _downloadFileChannel = MethodChannel(
  'codexflow/download_file',
);
    ...
    return PanelCard(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          const SizedBox(height: 12),
          if (artifact.prefersPreview)
            Row(
              children: <Widget>[
                Expanded(child: ArtifactOpenButton(artifact: artifact)),
                const SizedBox(width: 10),
                Expanded(child: ArtifactDownloadButton(artifact: artifact)),
              ],
            )
          else
...
class ArtifactOpenButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ActionButton(
      title: artifact.prefersPreview ? '预览' : '打开',
...
class ArtifactDownloadButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ActionButton(
      ...
      padding: EdgeInsets.symmetric(vertical: compact ? 8 : 10),
      onPressed: () {
        unawaited(downloadArtifact(context, artifact));
      },
    );
  }
}
...
Future<void> openArtifact(BuildContext context, ArtifactRef artifact) async {
  ...
  final rawUrl = artifact.prefersPreview
      ? artifact.previewUrl
      : artifact.downloadUrl;
  bool opened = false;
  try {
    opened =
        await _openUrlChannel.invokeMethod<bool>('openUrl', <String, String>{
          'url': model.resolveAgentUrl(rawUrl),
        }) ??
        false;
  } catch (_) {
    opened = false;
  ...
  if (!opened && context.mounted) {
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('无法打开 ${artifact.name}')));
  }
}
...
Future<void> downloadArtifact(
  ...
  if (context.mounted) {
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('正在下载 ${artifact.name}…')));
  }
  try {
    final result = await _downloadFileChannel
        .invokeMethod<dynamic>('downloadFile', <String, String>{
          'url': model.resolveAgentUrl(artifact.downloadUrl),
    ...
    if (result is bool) {
      ...
      downloaded = result['ok'] == true;
      savedPath = result['path']?.toString() ?? '';
  ...
  if (downloaded) {
    ...
    final message = errorMessage.isEmpty
        ? '无法下载 ${artifact.name}'
        : '无法下载 ${artifact.name}：$errorMessage';
    messenger.showSnackBar(SnackBar(content: Text(message)));
  }
}
