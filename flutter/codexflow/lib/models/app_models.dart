...
class UploadedFileRef {
  UploadedFileRef({required this.id, required this.name, required this.size});
...
class ArtifactRef {
  ...
  factory ArtifactRef.fromJson(Map<String, dynamic> json) {
    return ArtifactRef(
      ...
      size: asInt(json['size']),
      modTime: parseDateTime(json['modTime']),
      downloadUrl: asString(json['downloadUrl']),
      previewUrl: asString(json['previewUrl']),
      sourceText: asString(json['sourceText']),
    );
  }

  String get displayPath => path.isEmpty ? name : path;
  ...
  String get kindLabel {
    switch (kind) {
      ...
      case 'archive':
        return '压缩包';
      default:
        return kind.isEmpty ? '文件' : kind;
    }
  }

  bool get prefersPreview => kind == 'pdf' || kind == 'image';
}
