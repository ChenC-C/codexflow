...
### 6.1 编译自己的 Android APK

如果你是从 GitHub 拉取源码并想生成自己的 Android 安装包，先准备：

- Flutter 3.41+ / Dart 3.11+
- JDK 17+
- Android SDK 与可用的 `adb`

然后执行：

```bash
git clone -b codexflow-share-clean-20260520 <repo-url>
cd codexflow/flutter/codexflow
flutter pub get
...
建议发布前把 `android/app/build.gradle.kts` 里的 `applicationId` 改成自己的唯一包名，并配置自己的 release keystore。当前工程为了方便本地编译测试，release 构建使用 debug signing config；这适合个人安装验证，不适合正式应用商店发布。
